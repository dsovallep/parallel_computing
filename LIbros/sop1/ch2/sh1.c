/*
     sh1.c  Simple shell que permite ver el uso de fork(), exec(), waitpid(), señales,
            asignacion de memoria dinamica (malloc(), free()), etc.
            TP II 
     UTN FRD
     SO
     Guillermo Cherencio
...
void inicio() {
   ...
   sh.input = STDIN_FILENO; // descriptor de archivo de stdin
   ...
   if (sh.interactivo) { // el shell es interactivo?
      ...
      while (tcgetpgrp(sh.input) != (sh.pgid = getpgrp())) 
