/*
   tp73.c IPC. Cola de mensajes. Message Queue. Programa servidor que lee de una cola
   ...
   Se hizo parsing de peticion, se proceso, se envio respuesta a cliente
         
   para compilar:
            gcc -Wall -o tp73 tp73.c myipc.c ../fifo/v3/myfifo.c -I ../fifo/v3
...
int main(int argc, char *argv[])  {
   ...
   if ((idcola = crear_cola(key)) != -1) { 
      ...
      char suc[SUC_MSG_LEN+1],pid[SUC_MSG_LEN+1];
      char *p;
      int i;
      while(!salir) {
         ...
         if ( recibir_msg(idcola,&qbuf,SUC_MSG_LEN,1L) != -1) {
            ...
            p=strcpyentre(pid,qbuf.mtext,'<','/');
            strcpyentre(suc,p,'/','>');
            i=atoi(suc);
            if ( i>= 0 && i < 10 ) {
               snprintf(qbuf.mtext,SUC_MSG_LEN,"<%d/%.2lf>",i,ventas[i]);
               if (enviar_msg(idcola,&qbuf,SUC_MSG_LEN,atol(pid)) != -1) {
                  printf("main():envie [%s] como respuesta a peticion\n",qbuf.mtext);
               }
            }
         }
      }
