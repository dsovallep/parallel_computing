/*
   tp68.c Pipes. FIFO. Programa servidor que lee aquello que los clientes
...
// formato de buffer: "<id proceso/nro de sucursal><id proceso/nro de sucursal>....<id proceso/nro de sucursal>"
void parse(char *buffer) {
   char id[MAX_BUFFER];
   char suc[MAX_BUFFER];
   int iid,isuc;
...
void proceso(int id,int suc) {
   ...
   //crearfifo(fifo); //se supone creado por el cliente
